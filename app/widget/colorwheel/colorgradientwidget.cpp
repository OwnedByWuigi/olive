...
void ColorGradientWidget::paintEvent(QPaintEvent *e)
{
	...
	if (orientation_ == Qt::Horizontal) {
		p.drawRect(qRound(width() * (1.0 - clamped_val)) - selector_radius, 0,
				   selector_radius * 2, height() - 1);
	} else {
		p.drawRect(0, qRound(height() * (1.0 - clamped_val)) - selector_radius,
				   width() - 1, selector_radius * 2);
	}
}
...
{
	float t =
		std::clamp(static_cast<float>(i) / static_cast<float>(max), 0.0f, 1.0f);

	return Color(lerp(a.red(), b.red(), t), lerp(a.green(), b.green(), t),
