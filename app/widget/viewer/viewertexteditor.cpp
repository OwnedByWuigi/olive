...
void ViewerTextEditor::Paint(QPainter *p, Qt::Alignment valign)
{
	...
	if (this->textCursor().hasSelection()) {
		...
		QPalette::ColorGroup cg = this->hasFocus() ? QPalette::Active :
													 QPalette::Inactive;
		...
		QStyleOption opt;
		opt.initFrom(this);
		if (this->style()->styleHint(QStyle::SH_RichText_FullWidthSelection,
									 &opt, this)) {
...
void ViewerTextEditor::DocumentChanged()
{
	...
	transparent_clone_->documentLayout()->setProperty(
		"cursorWidth", document()->documentLayout()->property("cursorWidth"));
...
void ViewerTextEditorToolBar::SetColor(const QColor &c)
{
	...
	color_btn_->setStyleSheet(
		QStringLiteral("QPushButton { background: %1; }").arg(c.name()));
}
