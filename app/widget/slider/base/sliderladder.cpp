...
SliderLadder::SliderLadder(double drag_multiplier, int nb_outer_values,
						   QString width_hint, QWidget *parent)
	: QFrame(parent, Qt::Popup)
{
	...
	for (int i = nb_outer_values - 1; i >= 0; i--) {
		elements_.append(new SliderLadderElement(
			qPow(10, i + 1) * drag_multiplier, width_hint));
	}
...
SliderLadderElement::SliderLadderElement(const double &multiplier,
										 QString width_hint, QWidget *parent)
	: QWidget(parent)
...
void SliderLadderElement::UpdateLabel()
{
	if (multiplier_visible_) {
		...
		label_->setText(QStringLiteral("%1\n%2").arg(
			QString::number(multiplier_), val_text));
	} else {
