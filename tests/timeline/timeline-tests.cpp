...
OLIVE_ADD_TEST(AddTrack)
{
...

	{
		...
		first_video_track = TimelineAddTrackCommand::RunImmediately(
			sequence.track_list(Track::kVideo));

		OLIVE_ASSERT(sequence.GetConnectedOutput(Sequence::kTextureInput) ==
					 first_video_track);
		OLIVE_ASSERT(sequence.track_list(Track::kVideo)->GetTrackCount() == 1);
		OLIVE_ASSERT(sequence.track_list(Track::kVideo)->GetTrackAt(0) ==
					 first_video_track);
	}

	{
		// Test creating initial audio track
		first_audio_track = TimelineAddTrackCommand::RunImmediately(
			sequence.track_list(Track::kAudio));

		OLIVE_ASSERT(sequence.GetConnectedOutput(Sequence::kSamplesInput) ==
					 first_audio_track);
		OLIVE_ASSERT(sequence.track_list(Track::kAudio)->GetTrackCount() == 1);
		OLIVE_ASSERT(sequence.track_list(Track::kAudio)->GetTrackAt(0) ==
					 first_audio_track);
	}

	{
		// Test creating second video track with merge
		Track *second_video_track = TimelineAddTrackCommand::RunImmediately(
			sequence.track_list(Track::kVideo), true);
		OLIVE_ASSERT(sequence.GetConnectedOutput(Sequence::kTextureInput) !=
					 first_video_track);
		OLIVE_ASSERT(sequence.GetConnectedOutput(Sequence::kTextureInput) !=
					 second_video_track);
		OLIVE_ASSERT(sequence.track_list(Track::kVideo)->GetTrackCount() == 2);
		OLIVE_ASSERT(sequence.track_list(Track::kVideo)->GetTrackAt(1) ==
					 second_video_track);

		MergeNode *merge = dynamic_cast<MergeNode *>(
			sequence.GetConnectedOutput(Sequence::kTextureInput));
		OLIVE_ASSERT(merge);
		OLIVE_ASSERT(merge->GetConnectedOutput(MergeNode::kBaseIn) ==
					 first_video_track);
		OLIVE_ASSERT(merge->GetConnectedOutput(MergeNode::kBlendIn) ==
					 second_video_track);
	}

	{
		// Test creating second audio track with merge
		Track *second_audio_track = TimelineAddTrackCommand::RunImmediately(
			sequence.track_list(Track::kAudio), true);
		OLIVE_ASSERT(sequence.GetConnectedOutput(Sequence::kSamplesInput) !=
					 first_audio_track);
		OLIVE_ASSERT(sequence.GetConnectedOutput(Sequence::kSamplesInput) !=
					 second_audio_track);
		OLIVE_ASSERT(sequence.track_list(Track::kAudio)->GetTrackCount() == 2);
		OLIVE_ASSERT(sequence.track_list(Track::kAudio)->GetTrackAt(1) ==
					 second_audio_track);

		MathNode *merge = dynamic_cast<MathNode *>(
			sequence.GetConnectedOutput(Sequence::kSamplesInput));
		OLIVE_ASSERT(merge);
		OLIVE_ASSERT(merge->GetConnectedOutput(MathNode::kParamAIn) ==
					 first_audio_track);
		OLIVE_ASSERT(merge->GetConnectedOutput(MathNode::kParamBIn) ==
					 second_audio_track);
	}
...
OLIVE_ADD_TEST(SequenceDefaults)
{
	...
	Track *tex_connect =
		dynamic_cast<Track *>(sequence.GetConnectedTextureOutput());
	OLIVE_ASSERT(tex_connect);
	Track *smp_connect =
		dynamic_cast<Track *>(sequence.GetConnectedSampleOutput());
